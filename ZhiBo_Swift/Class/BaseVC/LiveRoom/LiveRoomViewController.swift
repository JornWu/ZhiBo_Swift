...
///
    ...
    ///     直播间数据
    ///
    private lazy var roomDataAr: [AnyObject] = {
        return [AnyObject]()
    }()
    ...
    ///     当前直播间数据模型（信号）
    ///
    public lazy var currentRoomModeSignalPipe: (output: Signal<AnyObject, NoError>, input: Observer<AnyObject, NoError>) = {
        return Signal<AnyObject, NoError>.pipe()
    }()
...
///
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: "LiveRoomCell", for: indexPath) as! LiveRoomCollectionViewCell
        cell.backgroundColor = UIColor.gray
        if let data = dataItem as? HL_List {
            cell.setupCell(withImageURLString: data.bigpic, flvLinkString: data.flv)
        }
        else if let data  = dataItem as? NR_List {
            cell.setupCell(withImageURLString: data.photo, flvLinkString: data.flv)
        }
